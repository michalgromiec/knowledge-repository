...
* columnar data storage (splittable)
* column compression - each compression can be compressed separately with different compression. This can lead to exception raised when column compressed with compression are tried to be promoted to sort key - sort keys can't be compressed.
